...
function addTab() {
    ...
    // const tabIndex = nextTabId; // Ya no lo necesitamos aquí
    nextTabId++;
    const tabButton = document.createElement('button');
    tabButton.id = tabId;
    tabButton.className = 'tab-button';
    // 1. Modificamos esta línea. Solo agregamos el botón.
    tabButton.innerHTML = `<button class="close-tab-btn" title="Cerrar pestaña">&times;</button>`;
    ...
    // 2. Agregamos la llamada a renumberTabs() ANTES de switchTab
    renumberTabs();
...
function closeTab(tabId) {
    // --- LÍNEA AÑADIDA ---
    if (!confirm('¿Estás seguro de que querés cerrar esta pestaña? Los datos no guardados se perderán.')) {
        return; // Si el usuario aprieta "Cancelar", no hace nada
    }
...
function generarCodigo() {
    ...
    if (modo_antonio_rossi) {
        ...
        filename = `${slug}.html`; urlIndex = `Noticias-${anio}${mes}/${filename}`; urlIndividual = `../../${urlIndex}`; carpetaIndex = 'Imagenes/'; imgPathIndividual = imgFilenamePrincipal ? `../../${carpetaIndex}${imgFilenamePrincipal}` : ''; imgPathIndex = imgFilenamePrincipal ? (carpetaIndex + imgFilenamePrincipal) : '';
    }
    canonicalUrl = `${BASE_URL}/${urlIndex}`;
...
function addImageFieldDirect(form, url = '', type = 'Transporte') {
    ...
    fieldWrapper.innerHTML = `
            <div class="image-input-controls">
                <select class="image-type-selector" name="imagen_adicional_type_${container.children.length}">
                    <option value="Transporte">Transporte</option>
                    <option value="Energia">Energia</option>
                    <option value="AR">Antonio Rossi (AR)</option>
                </select>
                <input type="url" class="image-url" name="imagen_adicional_url_${container.children.length}" placeholder="https://...">
                <button type="button" class="show-name-btn">Mostrar Nombre</button>
            </div>
            <div class="image-input-feedback">
                <div class="image-preview-container">
                    <img src="#" alt="Vista previa adicional" class="image-preview hidden">
